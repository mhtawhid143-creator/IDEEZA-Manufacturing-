...
const sha = execFileSync('git', ['ls-remote', REPO, 'refs/heads/main'], { encoding: 'utf8' })
  .split('\t')[0]
  .trim();

const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'ideeza-ds-'));

// A shallow clone rather than curl piped into tar: it needs no shell, so it
// runs on every platform this repository is developed on, and it asks for the
// commit by name instead of trusting a redirect to return the right tarball.
execFileSync('git', ['clone', '--quiet', '--filter=blob:none', '--no-checkout', REPO, tmp], {
  stdio: ['ignore', 'ignore', 'inherit'],
});
execFileSync('git', ['-C', tmp, 'fetch', '--quiet', 'origin', sha], { stdio: 'inherit' });
execFileSync('git', ['-C', tmp, 'checkout', '--quiet', sha], { stdio: 'inherit' });
...
// The merger the components use is replaced on the way in. The design repo
// ships a plain `twMerge`, which cannot know this project's text-style names
// and therefore mistakes `text-label-lg` for a colour — dropping the button
// label colour it was merged with. See tools/ds-cn.template.ts.
fs.copyFileSync(path.join(root, 'tools', 'ds-cn.template.ts'), path.join(target, 'lib', 'cn.ts'));
