/**
 * Screenshots of the manufacturer panel, desktop and phone, in one pass.
 *
 * The verification harness already boots a real stack and takes pictures, but
 * it takes the ones its checks happen to need and spends nine minutes proving
 * behaviour. A design pass wants the opposite: every surface, both widths, no
 * assertions. So this reuses the harness's boot exactly — same migrations, same
 * seed, same fixtures, same built server — and only looks.
 *
 *   node tools/shoot-manufacturer.mjs
 */
...
const shotDir = join(root, '.design-shots-manufacturer');
...
/** Every surface a shop actually works in, in the order it meets them. */
const SURFACES = [
  ['dashboard', '/dashboard'],
  ['rfq-inbox', '/rfqs'],
  ['rfq-brief', '/rfqs/mfrfix_rfq_driver'],
  ['rfq-files', '/rfqs/mfrfix_rfq_driver/files'],
  ['rfq-spec', '/rfqs/mfrfix_rfq_driver/specification'],
  ['rfq-bom', '/rfqs/mfrfix_rfq_driver/bom'],
  ['quotes', '/quotes'],
  ['quote-detail', '/quotes/verify_quote_a'],
  ['orders', '/orders'],
  ['order-detail', '/orders/mfrfix_order_beacon'],
  ['order-quote', '/orders/mfrfix_order_beacon/quote'],
  ['order-files', '/orders/mfrfix_order_beacon/files'],
  ['inventory', '/inventory'],
  ['payouts', '/payouts'],
  ['messages', '/messages'],
  ['profile', '/profile'],
...
const main = async () => {
  takeBuildLock('manufacturer screenshots');
  const dataDir = mkdtempSync(join(tmpdir(), 'ideeza-shoot-mfr-pg-'));
  ...
  try {
    ...
    await postgres.createDatabase('ideeza_shoot_mfr');
    const databaseUrl = `postgresql://postgres:postgres@127.0.0.1:${pgPort}/ideeza_shoot_mfr?schema=public`;
    ...
    run('shop fixtures', 'verify-fixtures-manufacturer.ts', { DATABASE_URL: databaseUrl });
    ...
    for (const [width, height, suffix] of [
      ...
      for (const [name, path] of SURFACES) {
        ...
        // The panels are wide reads and a few settle after hydration; a short
        // pause buys a picture of the page rather than of it arriving.
