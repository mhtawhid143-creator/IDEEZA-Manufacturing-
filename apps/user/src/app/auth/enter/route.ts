 ...
 *
 * `?as=<email>` names the buyer to enter as; without it the account with the
...
const safeNext = (value: string | null): string =>
  value !== null && value.startsWith('/') && !value.startsWith('//') ? value : '/manufacturing';
...
export const GET = async (request: NextRequest): Promise<NextResponse> => {
  ...
  if (account === undefined) {
    // Either the database has no buyer, or the address is not one of them. Both
